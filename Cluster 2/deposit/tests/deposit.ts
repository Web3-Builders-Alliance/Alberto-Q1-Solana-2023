...
describe("deposit", () => {
  ...
  xit("Withdraws SPL Token", async () => {
    let from_token_acct = getAssociatedTokenAddressSync(
      mint.publicKey,
      pda_auth,
      true,
      TOKEN_PROGRAM_ID,
    ...
    let to_token_acct = getAssociatedTokenAddressSync(
      mint.publicKey,
    ...
    let withdraw_spl_tx = await program.methods.withdrawSpl(new anchor.BN(1e6)).accounts({
      ...
      fromTokenAcct: from_token_acct,
      toTokenAcct: to_token_acct,
      assiciatedTokenProgram: ASSOCIATED_TOKEN_PROGRAM_ID,
      tokenProgram: TOKEN_PROGRAM_ID,
      systemProgram: anchor.web3.SystemProgram.programId,
    }).signers([deposit_auth]).rpc();
